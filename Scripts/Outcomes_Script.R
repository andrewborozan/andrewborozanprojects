...
a <- q6sentiment_comments_tab
a
a %<>% select(-sd)
a <- data.frame(a = c(1:3), b = c(4:6), c= c(7:9))
a
a %<>% select(-sd)
a

ikea %>% group_by(sellable_online, category) %>% 
  summarize(n = n()) %>% filter(sellable_online != "TRUE") %>% 
  arrange(desc(n))
...
ikea %>% group_by(category) %>% 
  summarize(n = n(), 
            avg_price = mean(price_usd)) %>% 
  mutate(category = fct_reorder(category, avg_price)) %>% 
  ggplot(aes(category, avg_price, fill = n)) +
  geom_col() +
