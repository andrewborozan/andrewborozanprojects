...
refactor <- function(x){
  q3 %>% mutate(x = factor(x, levels = c("Not Effective at all", "Slightly Effective", "Somewhat Effective", "Effective", "Extremely Effective")))
}
...
q3_tab <- function(col, question){
  return(survey_clean %>% select(starts_with("q3")) %>% count_(col) %>% mutate(Number_of_Responses = n,  percent_respondents = percent(n/sum(n), accuracy = .1)) %>% 
