...
library(pacman)
p_load(tidyverse, skimr, scales, rmarkdown, magrittr, lubridate, janitor, htmltab, ggrepel, viridis, ggthemes, knitr, reactable, stringr.plus, ggridges, readxl, reactablefmtr, showtext, ggpubr, sentimentr, tm, plotly, tinytex, webshot)
options(scipen=999)
...
q3_tab <- function(col, question){
  return(survey_clean %>% select(starts_with("q3")) %>% count_(col, sort = TRUE) %>% mutate(Number_of_Responses = n,  percent_respondents = percent(n/sum(n), accuracy = .1)) %>% 
